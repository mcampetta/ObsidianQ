...
fn stop_event_name(drive: char) -> Vec<u16> {
    ffi::str_to_wchar_vec(&format!("Global\\ObsidianQ_Mount_{}", drive.to_ascii_uppercase()))
}
...
pub fn unmount_container(drive_letter: char) -> Result<()> {
    ...
    #[cfg(feature = "winfsp")]
    {
        ...
        let event_name = stop_event_name(dl);
        let h = unsafe {
            OpenEventW(EVENT_MODIFY_STATE, 0, event_name.as_ptr())
        };
...
#[cfg(feature = "winfsp")]
// Store the stop-event handle in a static so the Ctrl+C callback can reach it.
// isize is the same size as HANDLE on all supported Windows targets.
static STOP_EVENT_HANDLE: std::sync::atomic::AtomicIsize =
...
unsafe extern "system" fn ctrl_handler(_ctrl_type: u32) -> windows_sys::Win32::Foundation::BOOL {
    ...
    if !h.is_null() {
        unsafe { windows_sys::Win32::System::Threading::SetEvent(h); }
    }
